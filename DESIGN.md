...
- **동작**: 빨강/노랑/초록 표시등은 각각 닫기/최소화/전체 화면 전환 버튼입니다. 일반 창에서는 타이틀바의 빈 영역을 드래그해 창을 이동하며 버튼과 알림 영역은 드래그 대상에서 제외합니다. 알림 버튼은 History의 Event 탭으로 이동하고, hover는 라벤더 15% 표면, focus는 공통 2px 라벤더 outline을 사용합니다.
