...
- **상태**: 알림 수는 API 연결 전까지 `1` 정적 값이며, 연결 영역에는 현재 선택된 시연/로컬 모드를 표시합니다. 시각은 로컬 시스템 시간을 사용합니다.
- **창 모드**: Electron 창은 프레임 없이 전체 화면으로 시작합니다. 초록 표시등으로 전체 화면을 켜고 끄며, 일반 창에서도 같은 타이틀바를 유지합니다.
- **동작**: 빨강/노랑/초록 표시등은 각각 닫기/최소화/전체 화면 전환 버튼입니다. 일반 창에서는 타이틀바의 빈 영역을 드래그해 창을 이동하며 버튼과 알림 영역은 드래그 대상에서 제외합니다. 연결 영역은 연결 설정 창을 열고, 알림 버튼은 History의 Event 탭으로 이동합니다. hover는 라벤더 15% 표면, focus는 공통 2px 라벤더 outline을 사용합니다.

### ConnectionSettingsDialog
- **구조**: 기존 ModuleModal의 modal surface, 경계선과 라벤더 focus 스타일을 재사용하며 시연/로컬 모드 선택과 로컬 REST·WebSocket 주소 입력을 표시합니다.
- **상태**: 초기값은 시연 모드입니다. 로컬 주소는 모드를 바꿔도 유지하고, 잘못된 주소나 저장 실패는 입력 영역에 텍스트로 알립니다.
- **동작**: 저장하면 선택값을 사용자 설정에 보관하고 화면을 새로고침해 모든 API·WebSocket 연결에 적용합니다. 취소하거나 Escape를 누르면 변경 없이 닫습니다.
